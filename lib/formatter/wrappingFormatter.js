...
function wrapString(value, builder, client) {
  ...
  // Fast path: simple identifier (no dot, no alias)
  if (dotIdx === -1 && spaceIdx === -1) {
    return wrapAsIdentifier(value, builder, client);
  }

  // Check for ' as ' alias only when a space is present
  if (spaceIdx !== -1) {
  ...
  // No dot — identifier that contains spaces but not ' as '
  if (dotIdx === -1) {
    return wrapAsIdentifier(value, builder, client);
  }

  // Dotted identifier (e.g. "schema.table" or "table.column")
  const segments = value.split('.');
  let result = wrapString((segments[0] || '').trim(), builder, client);
  for (let i = 1; i < segments.length; i++) {
    result += '.' + wrapAsIdentifier(segments[i], builder, client);
  }
  return result;
}
