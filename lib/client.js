...
class Client extends EventEmitter {
  ...
  parameterize(values, notSetValue, builder, bindingsHolder) {
    ...
    // Fast path: when all values are primitives (no Raw, QueryBuilder, or
    // plain objects needing JSON serialization), skip per-element parameter()
    // calls and build the placeholder string in bulk.
    let allSimple = true;
    for (let i = 0; i < len; i++) {
      const v = values[i];
      const type = typeof v;
